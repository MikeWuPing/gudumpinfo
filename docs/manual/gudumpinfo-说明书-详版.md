...
下表列出 19 个已实现视图与 UEFI Shell 命令的对应关系，其中标"（无 Shell 命令）"的视图是 Shell 原生环境难以完成或需要专门工具才能获取的信息，正是本工具的价值所在。
...
| Event/Timer | （无 Shell 命令） | 全系统事件与定时器扫描：事件对象、通知回调、挂载协议、周期定时器 |
...
![Event/Timer 视图总览](images/ev_overview.png)

Event/Timer 视图扫描并展示 UEFI 事件系统（EDK2 `IEVENT` 对象）的全部注册事件与定时器，这是 Shell 命令无法触及的信息面。进入视图后自动进行内存扫描（顶部进度条显示进度，完成后隐藏），扫描基于 EDK2 事件对象的 `'evnt'` 签名与多层结构校验，并用自校准事件验证当前固件内核是否兼容（不兼容时给出低置信提示，结果仍可参考）。

![Event/Timer 详情](images/ev_detail.png)

左侧列表分两组：**Event**（普通事件，含组事件与协议通知事件）与 **Timer**（周期定时器），组头显示条目数并可双击折叠。每行显示事件地址、通知级别（notify/callback/app 短名）与摘要——摘要按优先级取事件组名（ReadyToBoot、ExitBootServices 等）、挂载协议名（Loaded Image 等）、或回调函数所属模块名（XhciDxe 等，通过 OVMF 构建的 GUID 映射表或 PDB 反查解析）。点击行后右侧详情逐字段展示：事件类型位解读、通知级别、通知函数地址及其所属模块、通知上下文、事件组 GUID、触发计数、ExFlag 标志，Timer 条目还给出周期估算、下次触发 tick 与挂载状态；详情下方一行是"查看句柄"链接，点击直接跳到 Handle 视图对应驱动句柄。

全局搜索框对 Event 视图同样生效，支持大小写不敏感的模糊匹配——搜 "exit" 即可命中 ExitBootServices 组事件，也可按协议名、模块名、通知级别过滤。视图内容可用 Ctrl+S 保存为 .gud 文件（记录 168 字节定长条目，含挂载协议 GUID），载入后离线复看。

## 六、开发中功能

按钮区第二排末尾还有五个占位按钮：SPD、SMBus/I2C、TPM、Dependency、EC。它们当前是可以点击的，点击后内容区显示居中的"正在开发中，敬请期待"提示。SPD 计划读取内存条 SPD 信息，SMBus/I2C 计划提供总线读写工具，TPM 计划展示可信平台模块的状态与 PCR 信息，Dependency 计划解析 DXE 驱动的 dependency 表达式，EC 计划访问嵌入式控制器的寄存器空间——这些能力都需要更深的平台协议支持，会在后续版本中陆续落地。

## 七、已知限制与后续路线图

当前版本是 0.1.331，属于功能已成形、边角仍在打磨的阶段，有几点使用中需要注意。其一，大数据量视图有条目上限（handle 最大 4096，变量 2048 等），达到上限时状态栏会提示"截断"，这是 UEFI 无虚拟内存环境下对渲染性能的主动取舍；搜索过滤同样有行数上限，但正常平台远不会触及。其二，内存编辑、IO 端口、MSR 三类写操作的效果依赖平台固件与硬件：MSR 的安全任意地址读需要固件提供 #GP 钩子支持，IO 端口窗口读取对副作用端口的提示依赖端口注解表（部分端口无注解），物理内存写穿到真实设备的行为因平台而异。其三，ACPI 表与内存的修改是"会话级"的——同会话启动操作系统生效，重启后恢复，不要把它当作持久化配置工具。其四，文件模式载入的是数据快照，载入后刷新（Ctrl+R）会退出文件模式回到实时采集，快照本身不会自动更新。其五，Event/Timer 视图的事件扫描依赖 EDK2 事件对象的内存布局：OVMF 与 AMI Aptio 系固件已验证通过，其他厂商固件由自校准机制兜底（低置信提示），扫描范围限定在 BootServices/运行时数据/常规内存区域。
